...
#Objectives Working with single row, multiple row and correlated subqueries. and Using the comparison operators and the ALL and NOT EXISTS operators with subqueries.

#Task 1: Write a SQL SELECT query to find all bookings that are due after the booking of the guest ‘Vanessa McCarthy’.

SELECT * FROM Bookings WHERE BookingSlot > (SELECT BookingSlot FROM Bookings WHERE BookingSlot = '15:00:00');
#Answer
...
#Answer
SELECT * FROM MenuItems WHERE Price > ALL (SELECT Price FROM MenuItems WHERE Type IN ('Starters', 'Desserts')); 
#Task 3: Write a SQL SELECT query to find the menu items that costs the same as the starter menu items that are Italian cuisine. exists

#SELECT * FROM MenuItems Where exists (Select * From Menus WHERE Menus.ItemID = MenuItems.ItemID AND Cuisine = "Italian" AND menuItems.Type = 'Starters');
#Answer
