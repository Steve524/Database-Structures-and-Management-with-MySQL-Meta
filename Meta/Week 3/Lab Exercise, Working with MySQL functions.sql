...
#Objectives Work with MySQL String, Math, Date and Comparison functions.
...
#answers
SELECT DATE_FORMAT(DeliveryDate,'%W') FROM mg_orders; 
#Task 3: Write a SQL SELECT query that calculates the cost of handling each order. This should be 5% of the total order cost. 
#Use an appropriate math function to round that value to 2 decimal places.

select OrderID, ROUND((Cost * 0.05),2) AS HandlingCost From mg_orders GROUP by OrderID; 

#answers
SELECT OrderID, ROUND((Cost * 5 / 100),2) AS HandlingCost FROM mg_orders; 
#Task 4: Review the query that you wrote in the second task. Use an appropriate comparison function 
#to filter out the records that do not have a NULL value in the delivery date column.

Select date_format(DeliveryDate, '%W')From mg_orders where ISNULL(DeliveryDate) = 0;
