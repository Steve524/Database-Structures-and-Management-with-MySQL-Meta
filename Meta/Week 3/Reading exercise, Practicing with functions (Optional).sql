...
FROM client_orders
WHERE ItemID = 4;
#Task 2: Format the afterDiscount column value from the earlier result for 5% discount in '#,###,###.##' format 
#rounded to 2 decimal places using the FORMAT function.
...
SELECT adddate(OrderDate, 30) FROM mg_orders;
...
SELECT OrderID, ItemID, Quantity, Cost, OrderDate, COALESCE(DeliveryDate, 'NOT DELIVERED') AS OrderStatus FROM mg_orders;
...
#Task 5: Generate data required for the report by retrieving a list of M&G orders yet to be delivered. 
#These orders have an 'In Progress' status using the NULLIF function.


SELECT OrderID, NULLIF(OrderStatus, 'In progress') AS InProgess FROM mg_orders; 
